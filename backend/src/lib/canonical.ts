...
const CANONICAL: readonly CanonicalSkill[] = [
  ...
  { slug: "nestjs", name: "NestJS", aliases: ["nest", "nest-js"] },
  {
    slug: "aspnet",
    name: "ASP.NET Core",
    aliases: ["asp-net", "asp-net-core", "aspnet-core"],
  },
  {
    slug: "dotnet",
    name: ".NET",
    aliases: ["net", "dot-net", "net-core", "dotnet-core"],
  },
  ...
  // --- Data --------------------------------------------------------------
  {
    slug: "postgresql",
    name: "PostgreSQL",
    aliases: ["postgres", "psql", "postgre-sql", "pg"],
  },
  ...
  { slug: "github-actions", name: "GitHub Actions", aliases: ["gh-actions"] },
  {
    slug: "cicd",
    name: "CI/CD",
    aliases: ["ci-cd", "ci", "continuous-integration", "continuous-delivery"],
  },
  ...
  // --- Web fundamentals --------------------------------------------------
  {
    slug: "http",
    name: "HTTP",
    aliases: ["https", "http-s", "http-https", "http-protocol"],
  },
  {
    slug: "dom",
    name: "The DOM",
    aliases: ["document-object-model", "the-dom"],
  },
  {
  ...
  },
  {
    slug: "oauth",
    name: "OAuth 2.0",
    aliases: ["oauth-2", "oauth-2-0", "oauth2"],
  },
  {
    slug: "oidc",
    name: "OpenID Connect",
    aliases: ["openid", "open-id-connect"],
  },
  ...
  // --- Practices ---------------------------------------------------------
  {
    slug: "testing",
    name: "Testing",
    aliases: ["software-testing", "automated-testing"],
  },
  { slug: "unit-testing", name: "Unit Testing", aliases: ["unit-tests"] },
  {
    slug: "e2e-testing",
    name: "End-to-End Testing",
    aliases: ["e2e", "end-to-end-testing", "e-2-e"],
  },
  {
    slug: "algorithms",
    name: "Algorithms",
    aliases: ["algorithms-and-data-structures", "dsa"],
  },
];
...
/** Trailing version markers: "Angular 21", "Vue 3", "Python 3.12". */
const TRAILING_VERSION = /-(?:v?\d+(?:-\d+)*)$/;

const stripNoise = (slug: string): string => {
  let parts = slug.split("-").filter((part) => part.length > 0);

  // Only from the edges. "Test Driven Development" must keep its middle.
  while (parts.length > 1 && EDGE_NOISE.has(parts[0] as string))
    parts = parts.slice(1);
  while (parts.length > 1 && EDGE_NOISE.has(parts[parts.length - 1] as string))
...
export const canonicalise = (raw: string): CanonicalName => {
  ...
  return {
    name: trimmed,
    slug: narrowed.length > 0 ? narrowed : slug,
    known: false,
  };
};
