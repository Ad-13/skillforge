...
import { filterExpansion, type ExpansionContext, type FilteredExpansion } from './expansion.guard.ts'
...
const buildUserPrompt = (input: ExpandInput, previousIssues: readonly string[]): string => {
  const lines = [
  ...
  if (input.context.siblingSlugs.length > 0) {
    lines.push(`ALREADY LISTED UNDER TARGET: ${input.context.siblingSlugs.join(', ')}`)
  }

  if (previousIssues.length > 0) {
    lines.push('', 'Your previous answer was unusable:', ...previousIssues.map((i) => `- ${i}`))
  }
...
export const expandNode = async (
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    const answer = await aiProvider.completeStructured(expansionSchema, 'Expansion', {
      system,
    ...
    issues =
      filtered.rejected.length > 0
        ? filtered.rejected.map((r) => `"${r.label}" was discarded: ${r.reason}`)
