...
export const filterExpansion = (
  ...
  for (const child of children) {
    ...
    if (slug === context.targetSlug) {
      rejected.push({
        label: child.label,
        reason: "repeats the node being expanded",
      });
      continue;
    }
    ...
    if (path.has(slug)) {
      rejected.push({
        label: child.label,
        reason: "already on the path from the root",
      });
      continue;
    }
    ...
    if (siblings.has(slug)) {
      rejected.push({
        label: child.label,
        reason: "already a child of this node",
      });
      continue;
    }
    ...
    if (seen.has(slug)) {
      rejected.push({
        label: child.label,
        reason: "duplicate within this answer",
      });
      continue;
    }
