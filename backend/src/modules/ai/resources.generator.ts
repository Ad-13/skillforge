...
const buildSystemPrompt = (language: string, scope: 'stage' | 'step'): string => {
...
export const generateStepResources = async (
  ...
  const answer = await aiProvider.completeStructured(stepResourcesSchema, 'StepResources', {
...
export const generateStageResources = async (
  ...
  const user = [
    `SKILL: ${input.skillName}`,
    `STAGE: ${input.stageTitle}`,
    ...(input.stageRationale ? [`WHY THIS STAGE: ${input.stageRationale}`] : []),
    '',
    'STEPS:',
