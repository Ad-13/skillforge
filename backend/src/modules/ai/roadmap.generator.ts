...
export const generateRoadmap = async (input: RoadmapInput): Promise<RoadmapResult> => {
  ...
  const user = [
    ...
    ...input.topics.map((topic) =>
      topic.summary ? `- ${topic.label} — ${topic.summary}` : `- ${topic.label}`,
    ),
  ].join('\n')

  const roadmap = await aiProvider.completeStructured(roadmapSchema, 'Roadmap', {
