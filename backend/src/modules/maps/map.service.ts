...
export const LENSES: readonly LensName[] = ['FOUNDATION', 'ANATOMY', 'ECOSYSTEM']
...
export const parseLens = (raw: unknown): LensName => {
  ...
  if (!found) {
    throw new BadRequestError(`Unknown lens. Use one of: ${LENSES.join(', ').toLowerCase()}`)
  }
  return found
}
...
interface SkillWithProgress {
  slug: string
  name: string
  roadmap: { stages: { title: string; steps: { completedAt: Date | null }[] }[] } | null
}
...
export const mapService = {
  async getForSkill(userId: string, slug: string, lens: LensName): Promise<SkillMapView | null> {
    ...
    return {
      ...
      root: buildTree(map.nodes, await this.linkedSkills(userId, map.nodes), toLinkedView(skill)),
  ...
  async linkedSkills(
    ...
    const ids = [
      ...new Set(nodes.map((n) => n.linkedUserSkillId).filter((id): id is string => id !== null)),
    ]
    if (ids.length === 0) return new Map()

    const skills = await skillRepository.listWithRoadmapByIdsForUser(userId, ids)
  ...
  async promote(
    ...
    if (node.skillMap.userSkill.userId !== userId) throw new NotFoundError('Node not found')
    ...
    const linked = await mapRepository.linkNodesBySlug(userId, slug, created.id)
    ...
    return { map, skillSlug: created.slug, alsoLinked: Math.max(0, linked - 1) }
  },

  async generate(userId: string, slug: string, lens: LensName): Promise<SkillMapView> {
    const skill = await skillRepository.findBySlug(userId, slug)
    if (!skill) throw new NotFoundError('Skill not found')
    ...
    return {
      ...
      generatedBy: model,
      root: buildTree(nodes, await this.linkedSkills(userId, nodes), toLinkedView(skill)),
  ...
  async expand(
    ...
    return {
      map: {
        ...
        generatedBy: model,
        root: buildTree(nodes, await this.linkedSkills(userId, nodes), toLinkedView(owner)),
        nodeCount: nodes.length,
      },
      added,
