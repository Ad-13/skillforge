...
} from "../skills/skill.repository.ts";
import {
  generateStageResources,
  generateStepResources,
} from "../ai/resources.generator.ts";
import {
  normaliseMarkdown,
  stripLeadingH1,
  titleFromMarkdown,
...
export const resourceService = {
  ...
  async forgeForStage(
    ...
    const stage = skill.roadmap?.stages.find(
    ...
    if (stage.steps.length === 0) {
      throw new BadRequestError(
        "This stage has no steps to find resources for.",
      );
    }
    ...
    const byStep = new Map<
      string,
      (typeof resources.steps)[number]["resources"]
    >();
    ...
    const added = [...byStep.values()].reduce(
  ...
  async getStep(
    ...
    const rows = (await resourceRepository.listForStep(
      stepId,
    )) as ResourceRow[];
  ...
  async forgeForStep(
    ...
    return {
      step: await this.getStep(userId, slug, stepId),
  ...
  async importNote(
    ...
    const title =
      input.title ?? titleFromMarkdown(input.content, input.filename);
    const normalised = normaliseMarkdown(input.content);
    const body = stripLeadingH1(normalised, title);

    if (body.trim().length === 0) {
      throw new BadRequestError(
        "That file has a title and nothing else in it.",
      );
    }
  ...
  async update(
    ...
    if (resource.userSkill.userId !== userId)
    ...
    const row = await resourceRepository.update(resourceId, {
      ...
      ...(input.content !== undefined
        ? { content: normaliseMarkdown(input.content) }
        : {}),
    });
